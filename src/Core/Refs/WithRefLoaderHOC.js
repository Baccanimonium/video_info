...
import { getRef } from "@/Core/Refs/service"
import { useWatch } from "@/Utils/hooks/useWatch"

const WithRefLoaderHoc = (Component) => {
  const WithRefLoader = ({
    reference, paramsMap, refParams, ID_ENTITY_OBJ, refLoader, disabled, formPayload, alwaysReload, preload, getValue,
    ...props
  }) => {
    ...
    const loadRef = async (v_find) => {
      try {
        const requestParams = {
          ...
          ...refParams,
          ...dynamicParams,
          v_id_entity: ID_ENTITY_OBJ
        }

        if (isAllDynamicParamsProvided) {
          if (!equal(requestParams, prevRefParams) || alwaysReload) {
            setLoadingState(true)
            const nextOptions = await refLoader({ reference, params: requestParams }, valueKey)
  ...
  WithRefLoader.propTypes = {
    ...
    ID_ENTITY_OBJ: PropTypes.oneOfType([PropTypes.string, PropTypes.number]),
