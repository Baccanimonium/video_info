...
const HideAndShowText = ({ className, value, numberOfCharactersDisplayed, buttonComponent: ButtonComponent }) => {
  const [showText, updateShowText] = useState(true)

  const valueOutput = useMemo(() => (showText && value.length > numberOfCharactersDisplayed
    ? `${value.slice(0, numberOfCharactersDisplayed)}...`
    : value),
  [showText, value, numberOfCharactersDisplayed])

  const hideText = useCallback(() => updateShowText(!showText), [showText, updateShowText])
  return (
